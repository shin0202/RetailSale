...
public class RetialSaleDbAdapter
{
    ...
    {
        return db.query(DATA_OPTION_TABLE,
                new String[]
    ...
    {
        if (db != null && db.isOpen()) return db.delete(ADD_CUSTOMER_TABLE, KEY_ADD_CUSTOMER_ID + "=" + rowId, null) > 0;
        else
            return false;
    }
    ...
    {
        if (db != null && db.isOpen()) return db.delete(DATA_OPTION_TABLE, KEY_DATA_OPTION_ID + "=" + rowId, null) > 0;
        else
            return false;
    }
    ...
    {
        if (db != null && db.isOpen()) return db.delete(ADD_CUSTOMER_TABLE, null, null) > 0;
        else
            return false;
    }
    ...
    public Cursor getOption(String rowId) throws SQLException
    {
        if (db.isOpen())
        {
            ...
            { KEY_DATA_OPTION_ID, KEY_DATA_OPTION_TYPE, KEY_DATA_OPTION_ALIAS, KEY_DATA_OPTION_SERIAL,
                    KEY_DATA_OPTION_NAME }, KEY_DATA_OPTION_ID + "=" + rowId, null, null, null, null, null);
            if (cursor != null)
            {
                return cursor;
            }
    ...
    public Cursor getOptionByOptionSerial(int optionSerial) throws SQLException
    {
        if (db.isOpen())
        {
            Cursor cursor = db.query(true, DATA_OPTION_TABLE, new String[]
            { KEY_DATA_OPTION_ID, KEY_DATA_OPTION_TYPE, KEY_DATA_OPTION_ALIAS, KEY_DATA_OPTION_SERIAL,
                    KEY_DATA_OPTION_NAME }, KEY_DATA_OPTION_SERIAL + "=" + optionSerial, null, null, null, null, null);
            if (cursor != null)
            {
                return cursor;
            }
    ...
    public boolean updateOption(int rowID, String optionType, String optionAlias, int optionSerial, String optionName)
    {
        if (db.isOpen())
        {
            ContentValues args = new ContentValues();
    ...
    public Cursor getInfoAndCreateDate(String info, String createDate) throws SQLException
    {
        if (db.isOpen())
        {
            Cursor cursor = db.rawQuery("SELECT * FROM mails WHERE " + KEY_ADD_INFO + " = " + "\"" + info + "\""
                    + " AND " + KEY_ADD_CREATE_DATE + " = " + "\"" + createDate + "\"", null);
            if (cursor != null)
            {
                cursor.moveToFirst();
            }
